    ...
    try:
        ...
        if options.get('ocr', False):
            ...
            try:
                ...
                result = rapid_ocr_pdf(current_file, output_pdf, dpi=300)
                
                if result["success"]:
                    current_file = output_pdf
                    temp_files.append(output_pdf)
                    device = result.get('device', 'CPU')
                    log_message(f"🔍 {filename}: AI OCR 完成 [{device}]（{result['pages']}页，{result['time']:.1f}秒）", 'success')
                else:
...
def get_status():
    ...
    # 实时计算已用时间
    elapsed = None
    if processing_status['start_time'] is not None:
        if processing_status['elapsed_time'] is not None:
            elapsed = processing_status['elapsed_time']
        else:
            elapsed = round(time.time() - processing_status['start_time'], 1)
    ...
    return jsonify({
        ...
        'logs': processing_status['logs'][-50:],
...
def health_check():
    """健康检查（含 GPU 状态）"""
    from core.rapid_ocr import detect_device
...
def process_files():
    ...
    try:
        ...
        processing_status['elapsed_time'] = None
        ...
        def process_in_background():
            ...
            try:
                ...
                max_workers = 3  # 3 线程并行（GPU 单卡上限）

                def process_single(input_path, idx):
                    """处理单个文件"""
                    # 检查停止标志
                    ...
                    try:
                        log_message(f"🔍 [{idx+1}/{len(saved_files)}] AI OCR 开始：{input_path.name}", 'info')
                        processing_status['current_file'] = input_path.name

                        output_pdf = output_dir / f'searchable_{input_path.stem}.pdf'
                        result = rapid_ocr_pdf(
                            input_path, output_pdf, dpi=300,
                            stop_check=lambda: not processing_status['is_processing']
                        )

                        if result["success"]:
                            log_message(
                                f"✅ [{idx+1}] {input_path.name}: 完成 "
                                f"[{result.get('device','CPU')}]（{result['pages']}页，{result['time']:.1f}秒）",
                                'success'
                            )
                            processing_status['success_count'] += 1
                        else:
                            ...
                            if err == '用户中止':
                                log_message(f"⏹️ [{idx+1}] {input_path.name}: 已中止", 'warning')
                            else:
                                log_message(f"❌ [{idx+1}] {input_path.name}: {err}", 'error')
                            processing_status['fail_count'] += 1

                    except Exception as e:
                        log_message(f"❌ [{idx+1}] {input_path.name}: {e}", 'error')
                        processing_status['fail_count'] += 1
                    finally:
                        processing_status['processed_files'] += 1
                        processing_status['progress'] = int(
                            (processing_status['processed_files'] / len(saved_files)) * 100
                        )
                        gc.collect()

                log_message(f"🚀 并行处理：{len(saved_files)} 个文件，{max_workers} 线程", 'info')

                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = {
                        executor.submit(process_single, path, idx): path
                ...
                if processing_status['start_time'] is not None:
                    processing_status['elapsed_time'] = round(time.time() - processing_status['start_time'], 1)
                log_message(f'🎉 全部完成！', 'success')
